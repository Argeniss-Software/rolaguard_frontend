...
class ListNotificationsComponent extends React.Component {
    ...
    loadPage() {
        ...
        this.props.notificationStore.query(pagination).then(
            ({ data, headers }) => {
                const notifications = data;
                const totalItems = headers['total-items'];
                const totalPages = headers['total-pages'];
                this.setState({isLoading: false, totalItems, totalPages, notifications});
    ...
    markAsRead = index => {
      ...
      this.props.notificationStore.update(notifications[index].id, {readAt: new Date().toISOString()}).then( () => {});
    ...
    markAsUnread = index => {
      ...
      this.props.notificationStore.update(notifications[index].id, {readAt: null}).then( () => {});
      this.setState({ notifications });
    }
    ...
    toggleRead = index => {
      ...
      this.props.notificationStore.update(notifications[index].id, {readAt: notifications[index].readAt}).then( () => {
        this.setState({ notifications });
      });
    ...
    showAlertDetails = (index) => {
      ...
      const selectedAlert = {
        ...
        isLast: this.state.pagination.page === this.state.totalPages && index === this.state.notifications.length-1
      }
    ...
    if (this.state.selectedAlert.index === 0 && direction < 0) {
        if (this.state.pagination.page > 1) {
          this.handlePaginationChange(null, {activePage: this.state.pagination.page-1})
        }
          
        return;
      }
  
      if (this.state.selectedAlert.index === this.state.notifications.length-1 && direction > 0) {
    ...
    render() {
        const { title, notifications, isLoading, totalPages, pagination, hasError, notificationDetailsLoadingID, selectedAlert, newNotifications } = this.state;
          ...
          <div className="view-body">
            <Table className="animated fadeIn" basic="very" compact="very">
              ...
              <Table.Body>
                ...
                {newNotifications && (
                  <Table.Row>
                    <Table.Cell colSpan='4' verticalAlign='middle' style={{textAlign: 'center'}}>
                      <Message info compact>
                        <Icon name='bell'/>
                        There're new notificacions&nbsp;&nbsp;<Button circular positive size='mini' icon='fas fa-sync' onClick={this.reload} content='Reload now'/>
                      </Message>
                    </Table.Cell>
                  </Table.Row>
                )}
                {!isLoading && notifications.length > 0 && (
                  notifications.map( (notification, index) => 
                    <Table.Row textAlign='left' key={index} style={{cursor: 'pointer'}}>
                      <Table.Cell className="notifications-table-cell risk-column" onClick={() => this.showAlertDetails(index)}>
                        <Label horizontal style={{backgroundColor: this.colors[notification.alertType.risk], color: 'white', borderWidth: 1, borderColor: this.colors[notification.alertType.risk], width: '100px'}}>{notification.alertType.risk}</Label>
                      </Table.Cell>
                      <Table.Cell className="notifications-table-cell" onClick={() => {this.markAsRead(index); this.showAlertDetails(index)}}>
                        {notification.alertType.name}
                      </Table.Cell>
                      <Table.Cell singleLine className="notifications-table-cell created-column">
                        <Moment format="YYYY-MM-DD HH:mm">{notification.createdAt}</Moment>
                      </Table.Cell>
                      <Table.Cell className="notifications-table-cell wd-xl td-actions">
                        <Popup trigger={
                            <button disabled={notification.id === notificationDetailsLoadingID}
                        ...
                        <Popup trigger={
                            <button  onClick={() => {this.toggleRead(index);}}>
                              <i className={`${notification.readAt ? 'far fa-envelope-open' : 'fas fa-envelope'}`}/>
                            </button>
                          }>
                          {`${notification.readAt ? 'Mark as unread' : 'Mark as read'}`}
                        </Popup>
                        <DeleteNotificationModal notification={notification} handleNotificationRemoval={this.handleDelete} />
                      </Table.Cell>
                    </Table.Row>
                  )
                )}
            ...
            {selectedAlert && <DetailsAlertModal loading={isLoading} alert={selectedAlert} showGoToAlerts={true} onClose={this.closeAlertDetails} onNavigate={this.goToNotification}/>}
          </div>
        </div>
      </div>
    );
