...
import {
  subscribeToUpdatedDataCollectorEvents,
  unsubscribeFromUpdatedDataCollectorEvents,
...
class DataCollectorListComponent extends React.Component {
  ...
  componentDidMount() {
    this.loadPage();
    this.loadActivities();
    this.subscriber = subscribeToUpdatedDataCollectorEvents((event) => {
      const id = event["id"];
  ...
  render() {
    ...
    let dataCollectorList = dataCollectorsOrdered.map(
      (dataCollector, index) => {
        return (
          <Table.Row key={dataCollector.name + index}>
            <Table.Cell>{dataCollector.name}</Table.Cell>
            <Table.Cell>
              {dataCollector.type ? dataCollector.type.name : ""}
            </Table.Cell>
            <Table.Cell>
              <span
                style={{
                  ...
                  width: 10,
                  backgroundColor:
                    dataCollector.status === "CONNECTED"
                      ? "#01dd01"
                      : dataCollector.status === "DISCONNECTED"
                      ? "red"
                      : "#c0c0c0",
                  borderRadius: "50%",
                  marginRight: 8,
            ...
            {!isLoadingActivity && (
              <Table.Cell>
                {dataCollector.lastMessage
                  ? moment(dataCollector.lastMessage).fromNow()
            ...
            <Table.Cell className="wd-xl">
              <div className="td-actions">
                ...
                {isAdmin && (
                  <ChangeStatusDataCollectorModal
                    dataCollector={dataCollector}
                  ...
                  <Popup
                    trigger={
                      <button
                        onClick={() =>
                          history.push(
                            `/dashboard/data_collectors/${dataCollector.id}/edit`
                          )
                        }
                      >
          ...
          <div className="view-header">
            ...
            <div className="view-header-actions">
              {isAdmin && (
                <div
                  id="new_data_source"
                  onClick={() => history.push("/dashboard/data_collectors/new")}
                >
