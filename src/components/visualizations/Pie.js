...
class Pie extends Component {
  ...
  getSVGHeight() {
    return (this.innerRadius * 2) + (this.legendItemsCount * this.legendEntryHeight) + this.toggleLegendHeight;
  }
  ...
  setSize() {
    if(this.refs.child) {
      ...
      this.width = this.refs.child.parentNode.clientWidth - 30;
  ...
  update() {
    ...
    const arcGroup = this.group
      .selectAll(".arc-group")
      .data(data, (d) => {
        return d.data.label;
      });

    const arcGroupEnter = arcGroup.enter()
      .append("g")
    ...
    merge.select("path")
      ...
      .duration(1000)
      .attrTween('d', (d) => {
        const currentArc = this.oldAngles[d.data.label] || {startAngle: 2*Math.PI, endAngle: 2*Math.PI};
    ...
    arcGroup
      ...
      .attrTween('d', (d) => {
        ...
        const interpolate = d3.interpolate(currentArc, {startAngle: 2*Math.PI, endAngle: 2*Math.PI});
      ...
      .on("end", (d) => {
        this.oldAngles[d.data.label] = {startAngle: 2*Math.PI, endAngle: 2*Math.PI};
      })
    ...
    arcGroup
      ...
      .duration(1000)
      .attr("x", (d) => {
        return this.createArc.centroid({startAngle: 2*Math.PI, endAngle: 2*Math.PI})[0];
      })
      .attr("y", (d) => {
        return this.createArc.centroid({startAngle: 2*Math.PI, endAngle: 2*Math.PI})[1];
      });
  ...
  setLegend() {
    ...
    filteredData = this.legendCollapsed ? this.props.data.slice(0, 4) : this.props.data;

    this.legendItemsCount = filteredData.length;
    // hide show more when all legends are visibles
    var visibility = this.legendCollapsed && this.props.data.length === filteredData.length ?
       "hidden" : "visible";
    this.svg.select('#legend-toggle')
      .style("visibility", visibility);

    const legendGroup = this.svg.select('#legend')
    ...
    legendGroupEnter
      .append('text')
      .append('tspan')
      .attr('class', (d) => d.selected ? 'legend-text selected' : 'legend-text')
      .attr('x', 15)
      .style('font-size', 12)
      .style('cursor', 'pointer')
