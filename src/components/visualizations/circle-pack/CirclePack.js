...
class CirclePackD3 {
  constructor(containerEl, props) {
    ...
    const pack = (data) =>
      d3.pack().size([props.width, props.height]).padding(3)(
        d3
          .hierarchy(data)
          .sum((d) => d.value)
    ...
    const root = pack({
      name: root,
      children: props.data.filter((item) => !item.selected),
    });
    ...
    for (const item of root.children) {
      ...
      if (text.node().getBoundingClientRect().width < 2 * item.r * 0.8) {
        text.attr(
          "transform",
          `translate(${
            item.x - text.node().getBoundingClientRect().width / 2
          },${item.y + text.node().getBoundingClientRect().height / 4})`
        );
      } else {
        text.remove();
      }
      ...
      const showTooltip = () => {
        ...
        if (!this.tooltip) {
          ...
          this.tooltip
            ...
            .style(
              "left",
              circle.node().getBoundingClientRect().left -
                tooltipDiv.node().getBoundingClientRect().width * 0.5 +
                item.r +
                "px"
            )
            .style(
              "top",
              circle.node().getBoundingClientRect().top + 1.5 * item.r + "px"
            );
        }
      };
      ...
      group.on("mouseover", showTooltip).on("mouseleave", removeTooltip);
    }
  }
