...
class NotificationsPreferencesComponent extends React.Component {
    ...
    this.props.notificationStore
      ...
      .then(({ data }) => {
        data.dataCollectors.sort((a, b) => {
          if (
            a.dataCollector.name.toLowerCase() >
            b.dataCollector.name.toLowerCase()
          ) {
            return 1;
          }
          if (
  ...
  toggleDestination = (destination) => {
    const { preferences } = this.state;
    const item = preferences["destinations"].find(
  ...
  onAdditionalPhoneAdded = () => {
    ...
    const smsItem = preferences.destinations.find(
