...
const TagSelector = (props) => {
  ...
  const handleSearch = (search) => {
    setTagsFiltered(
      tags.filter((tag) => tag.name.includes(search.target.value))
    );
  ...
  const handleOpen = () => {
    setOpen(true);
    setLoading(true);
    tagsStore.getTags().then((response) => {
      const tagsRetreived = response.data
        .filter(
          (tag) => !props.alreadyAssignTags.map((t) => t.id).includes(tag.id)
        )
    ...
    <React.Fragment>
      ...
      >
        <span className="tag-selector-popup">
          ...
          {/* Loading state */}
          {loading && (
            <div className="animated fadeInDown loading-message loading-tags">
