...
class UsersNewComponent extends React.Component {
  ...
  getData = () => {
    const userDataPromise = this.props.usersStore.getUserByUsernameApi(
      this.props.match.params.username
    );
    const userRolesPromise = this.props.rolesStore.getRolesApi();

    Promise.all([userDataPromise, userRolesPromise]).then((response) => {
  ...
  validateForm = () => {
    ...
    formHasError =
      showErrors.emailTooLong || showErrors.fullNameTooLong ? true : false;
  ...
  roleDropdownDisabled() {
    return (
      ...
      !Validation.isUserAdmin(this.props.usersStore.currentUser)
    );
  }
