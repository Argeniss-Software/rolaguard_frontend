...
import {Segment, Grid, Table, Pagination, TableCell, Popup} from "semantic-ui-react";
...
const ResourceUsageComponent = (props) => {
	...
	const showStateIcon = (state, lastConnection) => {
		...
		if (state.trim().toLowerCase() === connectedState)
			return(
				<span><i className="fas fa-check-circle"></i> connected</span>
			);

		if (state.trim().toLowerCase() === disconnectedState)
			return(
				<span><i className="fas fa-times-circle"></i> disconnected</span>
			);
	}
	...
	const showMessagesSummary = (received, sended, lost) => {
		...
		return(
			<span>
				<Popup
					trigger={<span>{`${received} / ${sended} / ${lost}`}</span>}
			...
			<div className="animated fadeIn animation-view">
				<div className="view-header">
					...
					<div className="view-header-actions">
						...
						{showFilters &&
							<div onClick={() => setShowFilters(false)} style={{color: 'gray'}}>
											...
											{showFilters && (
												<div>
													<label style={{fontWeight: 'bolder'}}>Filters: </label>
														<span className="range-select" onClick={() => clearFilters()}>Clear</span>
												</div>
												)}
												{!listState.isLoading &&
													<Table className="animated fadeIn" basic="very" compact="very">
														...
														<Table.Header>
															<Table.Row>
																<Table.HeaderCell style={{cursor: "pointer"}} onClick={() => toggleDeviceType(criteria.type)}collapsing>
                                    {showDeviceTypeIcon(criteria.type)}
                                </Table.HeaderCell>
																<Table.HeaderCell collapsing>ID</Table.HeaderCell>
																<Table.HeaderCell>NAME</Table.HeaderCell>
																<Table.HeaderCell>STATE</Table.HeaderCell>
																<Table.HeaderCell>MESSAGES <i>(R/S/L)</i></Table.HeaderCell>
														...
														{listState.count > 0 &&
															<Table.Body>
																{!listState.isLoading && listState.data && (
																	listState.data.map((item, index) => {
																		return (
																			<Table.Row key={index}  style={{cursor: 'pointer'}}>
																				<Table.Cell style={{textAlign:"center"}}>{showDeviceTypeIcon(item.type)}</Table.Cell>
																				<Table.Cell>{item.hex_id}</Table.Cell>
																				<Table.Cell>{item.name}</Table.Cell>
																				<Table.Cell>{showStateIcon(item.state)}</Table.Cell>
																				<Table.Cell>{showMessagesSummary(item.received, item.sended, item.lost)}</Table.Cell>
												...
												{!listState.isLoading && listState.pagesCount > 1 && (
													<Grid className="segment centered">
														<Pagination className="" activePage={listState.activePage} onPageChange={handlePaginationChange} totalPages={listState.pagesCount} />
													</Grid>
												)}
									</Segment>
