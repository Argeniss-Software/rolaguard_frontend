          ...
          <Popup
            trigger={
              <button onClick={this.handleOpen} disabled={!isAdmin}>
                <i className={ buttonLoading ? "fas fa-spinner fa-spin square outline" : "fas fa-clipboard-check rectangle" } />
              </button>
            }
        ...
        <Table className="animated fadeIn" basic definition compact>
            <Table.Body>
              ...
              <Table.Row>
                ...
                <Table.Cell><Label horizontal style={{backgroundColor: AlertUtil.getColorsMap()[alert_type.risk], color: 'white', borderWidth: 1, borderColor: AlertUtil.getColorsMap()[alert_type.risk]}}>{alert_type.risk}</Label></Table.Cell>
              </Table.Row>
              <Table.Row>
          ...
          <Form>
            <TextArea placeholder='Leave a comment' value={comment} onChange={this.handleChange} style={{resize: 'none'}}/>
          </Form>
        </Modal.Content>
        <Modal.Actions>
          <Button
