...
class EmailActivation extends React.Component {
    ...
    componentWillMount() {
        ...
        this.props.notificationStore.activateEmail(token).then(
            res => this.setState({ email: res.data.email, isLoading: false, result: 'SUCCESS' })
        ).catch(
            err => {
                    ...
                    <Segment style={{minHeight: 300}}>
                        ...
                        {!isLoading && result === 'SUCCESS' &&
                            <div style={{marginTop: 25}}>
                                ...
                                <h3>The email {email} has been activated successfully</h3>
                            </div>
                        }
                        ...
                        {!isLoading && (result === 'EMAIL_ALREADY_ACTIVE') &&
                            <div style={{marginTop: 25}}>
                                ...
                                <h3>That email was activated previously</h3>
                            </div>
                        }
                        <BackToLogin/>
                    </Segment>
